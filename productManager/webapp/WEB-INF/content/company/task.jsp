...
<body>
<div id="menu">
<h3><a href="${ctx}/security/user.action">帐号列表</a> <a
	href="${ctx}/security/role.action">角色列表</a> <a
	href="${ctx}/company/company.action">公司列表</a> <a
	href="${ctx}/j_spring_security_logout">退出登录</a></h3>
</div>

<div id="message"><s:actionmessage theme="mytheme" /></div>
<form id="mainForm" action="task.action" method="get">
<div id="filter">产品名称: <input type="text"
	name="filter_LIKE_productName"
	value="${param['filter_LIKE_productName']}" size="9" /> <input
	type="button" value="搜索" onclick="search()" /></div>
		...
		<table width="100%">
			...
			<tr>
				<c:forEach var="item" items="${processList}">
					...
					<td width="33%"><c:out value="${item.processName}" /></td>
				...
				<tr>
					<c:forEach var="item" items="${processList}">
						...
						<td width="33%"><c:choose>
							<c:when test="${! empty processTaskMap[item]}">
				<a href="process-task!input.action?id=${processTaskMap[item].id}&taskId=${id}&processId=${item.id}">${processTaskMap[item].finishQuantity}</a>
			</c:when>
							<c:otherwise>
				<td>&nbsp; <security:authorize ifAnyGranted="A_MODIFY_USER">
				<a href="process-task!input.action?taskId=${id}&processId=${item.id}">0</a>
...
<div id="footer">第${page.pageNo}页, 共${page.totalPages}页 <a
