...
public class MediaViewTag extends BaseTag {
	...
	public int doStartTag() throws JspException {
		...
		if(browserInfo.indexOf("Opera Mobi") >= 0 
			...
			|| browserInfo.indexOf("fox") >= 0
			){	
				try{
					//response.sendRedirect("mView.jsp");
					request.getSession().getServletContext().getRequestDispatcher("/WEB-INF/visitor/m.jsp").forward(request,response);
				}catch(Exception e){
