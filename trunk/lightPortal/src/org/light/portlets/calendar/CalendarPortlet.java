	 ...
	 public void processAction (ActionRequest request, ActionResponse response) 
	    throws PortletException, java.io.IOException {
		 ...
		 if("save".equals(action)){
			 ...
			 if(location != null) location = URLDecoder.decode(location,_CHARSET_UTF);
			 ...
			 CalendarEvent event = new CalendarEvent(name,location,desc,start.getTime(),startTime,start.getTime(),endTime,user.getId(),state);
				 ...
				 if(end.after(start)){
					 ...
					 if(status == 1){
						 ...
						 while(end.after(next)){
							 CalendarEvent eventNext = new CalendarEvent(name,location,desc,next.getTime(),startTime,next.getTime(),endTime,user.getId(),state,event.getId());
...
//			 	request.getPortletSession().setAttribute("currentCalendar",start,PortletSession.PORTLET_SCOPE);					
	   ...
	   throws PortletException, java.io.IOException
	 {
		 ...
		 if(id != null){
			...
			if(eventId <= 0){
				 event = new CalendarEvent(current.getTime(),startTime,current.getTime(),endTime,userId,calendar.getState());
			 }else{
	 ...
	 private void prepareCalendar(RenderRequest request, RenderResponse response, CalendarBean calendar ,long userId){
		...
		String title = DateFormatter.format(current.getTime(),"EEEE, MMMM dd, yyyy");
		request.setAttribute("title",title);

		Calendar first = Calendar.getInstance();		
		first.setTimeInMillis(current.getTimeInMillis());
		String today = DateFormatter.format(first.getTime(),"EEEE, MMMM dd, yyyy");
		request.setAttribute("today",today);		
		List<CalendarEvent> events = this.getCalendarService(request).getCalendarEventsByDate(first.getTime(),userId);
		request.setAttribute("events",events);
		
		first.add(Calendar.DATE,  1 );
		String today1 = DateFormatter.format(first.getTime(),"EEE, MMM dd");
		request.setAttribute("today1",today1);	
		List<CalendarEvent> events1 = this.getCalendarService(request).getCalendarEventsByDate(first.getTime(),userId);
		request.setAttribute("events1",events1);
		
		first.add(Calendar.DATE,  1 );
		String today2 = DateFormatter.format(first.getTime(),"EEE, MMM dd");
		request.setAttribute("today2",today2);
		List<CalendarEvent> events2 = this.getCalendarService(request).getCalendarEventsByDate(first.getTime(),userId);
		request.setAttribute("events2",events2);
		
		first.add(Calendar.DATE,  1 );
		String today3 = DateFormatter.format(first.getTime(),"EEE, MMM dd");
		request.setAttribute("today3",today3);	
		List<CalendarEvent> events3 = this.getCalendarService(request).getCalendarEventsByDate(first.getTime(),userId);
		request.setAttribute("events3",events3);
		
		first.add(Calendar.DATE,  1 );
		String today4 = DateFormatter.format(first.getTime(),"EEE, MMM dd");
		request.setAttribute("today4",today4);	
		List<CalendarEvent> events4 = this.getCalendarService(request).getCalendarEventsByDate(first.getTime(),userId);
		request.setAttribute("events4",events4);
		
		first.add(Calendar.DATE,  1 );
		String today5 = DateFormatter.format(first.getTime(),"EEE, MMM dd");
		request.setAttribute("today5",today5);	
		List<CalendarEvent> events5 = this.getCalendarService(request).getCalendarEventsByDate(first.getTime(),userId);
		request.setAttribute("events5",events5);
		
		first.add(Calendar.DATE,  1 );
		String today6 = DateFormatter.format(first.getTime(),"EEE, MMM dd");
		request.setAttribute("today6",today6);	
		List<CalendarEvent> events6 = this.getCalendarService(request).getCalendarEventsByDate(first.getTime(),userId);
		request.setAttribute("events6",events6);
			
		first.add(Calendar.DATE,  1 );
		String today7 = DateFormatter.format(first.getTime(),"EEE, MMM dd");
		request.setAttribute("today7",today7);	
		List<CalendarEvent> events7 = this.getCalendarService(request).getCalendarEventsByDate(first.getTime(),userId);
	 ...
	 private void prepareCalendarDay(RenderRequest request, RenderResponse response, CalendarBean calendar, long userId){		
		...
		if(this.getPortlet(request) != null)
			request.getPortletSession().setAttribute("currentCalendar",current,PortletSession.PORTLET_SCOPE);			
		List<CalendarEvent> events = this.getCalendarService(request).getCalendarEventsByDate(current.getTime(),userId);
		//request.setAttribute("events",events);
		String title = DateFormatter.format(current.getTime(),"EEEE, MMMM dd, yyyy");
		request.setAttribute("title",title);
	 ...
	 private void prepareCalendarWeekDay(RenderRequest request, RenderResponse response, CalendarBean calendar ,long userId){
		...
		List<CalendarEvent> events2 = this.getCalendarService(request).getCalendarEventsByDate(monday.getTime(),userId);
		
		Calendar tuesday = Calendar.getInstance();		
		tuesday.setTimeInMillis(monday.getTimeInMillis());
		tuesday.add(Calendar.DATE,  1);		
		List<CalendarEvent> events3 = this.getCalendarService(request).getCalendarEventsByDate(tuesday.getTime(),userId);
		
		Calendar wednesday = Calendar.getInstance();		
		wednesday.setTimeInMillis(monday.getTimeInMillis());
		wednesday.add(Calendar.DATE,  2);		
		List<CalendarEvent> events4 = this.getCalendarService(request).getCalendarEventsByDate(wednesday.getTime(),userId);
		
		Calendar thursday = Calendar.getInstance();		
		thursday.setTimeInMillis(monday.getTimeInMillis());
		thursday.add(Calendar.DATE,  3);		
		List<CalendarEvent> events5 = this.getCalendarService(request).getCalendarEventsByDate(thursday.getTime(),userId);
		
		Calendar friday = Calendar.getInstance();		
		friday.setTimeInMillis(monday.getTimeInMillis());
		friday.add(Calendar.DATE,  4);		
		List<CalendarEvent> events6 = this.getCalendarService(request).getCalendarEventsByDate(friday.getTime(),userId);
		
		String title = DateFormatter.format(monday.getTime(),"EEE, MMM dd, yyyy")+" - "+DateFormatter.format(friday.getTime(),"EEE, MMM dd, yyyy");
		request.setAttribute("title",title);
		int start = calendar.getStartTime();
		int end = calendar.getEndTime();
		int interval = calendar.getInterval();
	 ...
	 private void prepareCalendarWeek(RenderRequest request, RenderResponse response, CalendarBean calendar ,long userId){
		...
		List<CalendarEvent> events2 = this.getCalendarService(request).getCalendarEventsByDate(second.getTime(),userId);
		
		Calendar third = Calendar.getInstance();		
		third.setTimeInMillis(first.getTimeInMillis());
		third.add(Calendar.DATE,  2);		
		List<CalendarEvent> events3 = this.getCalendarService(request).getCalendarEventsByDate(third.getTime(),userId);
		
		Calendar fourth = Calendar.getInstance();		
		fourth.setTimeInMillis(first.getTimeInMillis());
		fourth.add(Calendar.DATE,  3);		
		List<CalendarEvent> events4 = this.getCalendarService(request).getCalendarEventsByDate(fourth.getTime(),userId);
		
		Calendar fifth = Calendar.getInstance();		
		fifth.setTimeInMillis(first.getTimeInMillis());
		fifth.add(Calendar.DATE,  4);		
		List<CalendarEvent> events5 = this.getCalendarService(request).getCalendarEventsByDate(fifth.getTime(),userId);
		
		Calendar sixth = Calendar.getInstance();		
		sixth.setTimeInMillis(first.getTimeInMillis());
		sixth.add(Calendar.DATE,  5);		
		List<CalendarEvent> events6 = this.getCalendarService(request).getCalendarEventsByDate(sixth.getTime(),userId);
		
		Calendar last = Calendar.getInstance();		
		last.setTimeInMillis(first.getTimeInMillis());
		last.add(Calendar.DATE,  6);	
		List<CalendarEvent> events7 = this.getCalendarService(request).getCalendarEventsByDate(last.getTime(),userId);
		
		String title = DateFormatter.format(first.getTime(),"EEE, MMM dd, yyyy")+" - "+DateFormatter.format(last.getTime(),"EEE, MMM dd, yyyy");
		request.setAttribute("title",title);
	 ...
	 private void prepareCalendarMonth(RenderRequest request, RenderResponse response, CalendarBean calendar ,long userId){
		...
		if(this.getPortlet(request) != null)
			request.getPortletSession().setAttribute("currentCalendar",current,PortletSession.PORTLET_SCOPE);								
		String title = DateFormatter.format(current.getTime()," MMMM, yyyy");
		request.setAttribute("title",title);
		...
		if(current.getFirstDayOfWeek() == Calendar.MONDAY){
			...
			for(int i= 1;i<=maxDayOfMonth;i++){				
				...
				if(this.getCalendarService(request).hasEvents(day.getTime(),this.getUser(request).getId()))
