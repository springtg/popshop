...
<c:if test='${deliciousTags != null}'>
 <c:forEach var="tag" items="${deliciousTags}" >
  <c:if test='${tag.expanded}'>
    ...
    <input type="image" title='<fmt:message key="portlet.label.popItem"/>' src="<%= request.getContextPath() %>/light/images/popular.gif" style='border: 0px;' height='16' width='16' onClick="javascript:popDeliItem(event,'<c:out value="${bookmark.id}"/>','<c:out value="${requestScope.responseId}"/>');"/>
	<input type="image" title='<fmt:message key="portlet.label.forwardItem"/>' src="<%= request.getContextPath() %>/light/images/forward.gif" style='border: 0px;' height='16' width='16' onClick="javascript:forwardDeliToFriend(event,'<c:out value="${bookmark.id}"/>','<c:out value="${requestScope.responseId}"/>');"/>
	</light:authenticateUser>
...
<a href='<c:out value="${bookmark.url}"/>' target='_blank'><c:out value="${bookmark.name}"/></a>
<light:authenticateOwner>
