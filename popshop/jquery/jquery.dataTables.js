	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnGatherData( oSettings )
		{
			...
			for ( var i=0 ; i<oSettings.aoColumns.length ; i++ )
			{
				...
				{
					if ( $('tfoot th:nth-child('+(i+1)+')', oSettings.nTable) )
					{
						oSettings.aoColumns[i].sFooter = $('tfoot th:nth-child('+(i+1)+')', oSettings.nTable).text();
					}
				}
		...
		function _fnDrawHead( oSettings, iSortCol )
		{
			...
			/* If there is a footer put the footer text in as well */
			for ( var i=0 ; i<oSettings.aoColumns.length ; i++ )
			{
				if ( oSettings.aoColumns[i].bVisible )
				{
					nTh = document.createElement( "th" );
					nTh.innerHTML = oSettings.aoColumns[i].sFooter;
					nTrFoot.appendChild( nTh );
				}
			}
			
			/* Add the new footer */
			var jqFoot = $('tfoot', oSettings.nTable);
			if ( typeof jqFoot[0] == 'object' )
			{ 
				$('tfoot', oSettings.nTable).html( '' )[0].appendChild( nTrFoot );
			}
