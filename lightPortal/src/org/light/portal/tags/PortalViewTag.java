 ...
 **/
public class PortalViewTag extends BaseTag {
	...
	public int doStartTag() throws JspException {		
	    ...
	    try{
	    	...
	    	list.add(new LabelBean("portalString",action.visitPortal(request,response).toString()));
	    	list.add(new LabelBean("pageRoot",action.getPortalTabsByVisit(request,response).toString()));
	    	List<PortalTab> tabs = getPortalService(request).getUserProfilePortalTab(user.getUserId());	    	
	    	if(tabs == null || tabs.size() == 0)
	    		tabs = getPortalService(request).getPortalTabByUser(_ROLE_PROFILE);	    	
	    	...
	    	list.add(new LabelBean("page"+defaultTab.getId()+"Portlets",action.getPortletsByVisitTab(request,response).toString()));
