...
public class RegistrationPortlet extends LightGenericPortlet {
		 ...
		 if("request".equals(action)){
			...
			request.setAttribute("success","your new temporary password has been sent to your email account successfully.");			 
			 ...
			 if(user != null){			 			
				 ...
				 PortalUserRole userRole = new PortalUserRole(this.getUser(request).getId(),_CHANNEL_FORUM);
	   ...
	   throws PortletException, java.io.IOException
	 {
		...
		if(step == null || step.equals("1"))
			this.getPortletContext().getRequestDispatcher(_PORTLET_JSP_PATH+"/registration/register1.jsp").include(request,response);
			...
			this.getPortletContext().getRequestDispatcher(_PORTLET_JSP_PATH+"/registration/register2.jsp").include(request,response);
		}else if( step.equals("3")){
			request.setAttribute("channels", channels);
			this.getPortletContext().getRequestDispatcher(_PORTLET_JSP_PATH+"/registration/register3.jsp").include(request,response);
		}else if(step.equals("4")){
			this.getPortletContext().getRequestDispatcher(_PORTLET_JSP_PATH+"/registration/register4.jsp").include(request,response);
		}
		else if(step.equals("0")){
			this.getPortletContext().getRequestDispatcher(_PORTLET_JSP_PATH+"/registration/register0.jsp").include(request,response);
		}
	}	
