...
<c:if test='${profile.photoUrl != null}'>
<img src='<%= request.getContextPath() %><c:out value="${profile.photoUrl}"/>' style='border: 0px;'  align="middle" width='<c:out value="${profile.photoSmallWidth}"/>' height='<c:out value="${profile.photoSmallHeight}"/>'/>
</c:if>
</a>
</td>
...
<c:if test='${requestScope.userCount % 5 != 0}'>
</tr>
</c:if>
</table>
</form>
