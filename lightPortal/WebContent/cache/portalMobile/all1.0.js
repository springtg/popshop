...
if(portal.allowSignIn){var varE=document.createElement('a');var u=Light.getCookie(Light._LOGINED_USER_ID);if(Light.getCookie(Light._LOGINED_USER_ID)!=null&&Light.getCookie(Light._LOGINED_USER_ID)!=""){varE.innerHTML=$('_MENU_SIGN_OUT').title;varE.href="javascript:void(0)";varE.onclick=function(){Light.logoutMobile();}
...
vBody.appendChild(vSpan);vBody.style.zIndex=++Light.maxZIndex;vBody.style.width=Light.portal.layout.maxWidth-10;vBody.style.marginLeft=Light.portal.layout.containerLeft;return vBody;}
Light.logoutMobile=function(){Light.deleteCookie(Light._LOGINED_USER_ID);Light.deleteCookie(Light._USER_ID);Light.deleteCookie(Light._CURRENT_TAB);var date=new Date();date.setFullYear(date.getFullYear()+1);Light.setCookie(Light._IS_SIGN_OUT,"true");Light.portal.needReload=true;var opt={method:'get',parameters:'',onSuccess:function(t){Light.responseLogoutMobile(t);},on404:function(t){alert('Error 404: location "'+t.statusText+'" was not found.');},onFailure:function(t){alert('Error '+t.status+' -- '+t.statusText);}}
Light.ajax.sendRequest(Light.portal.contextPath+Light.logoutRequest,opt);}
Light.responseLogoutMobile=function(t){window.location.reload(true);}
